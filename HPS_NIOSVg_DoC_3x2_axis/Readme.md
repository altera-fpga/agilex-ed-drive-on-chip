...
This repository contains the necessary files and collateral to build and create a Quartus Prime Project variant of the 6-Axis Drive-On-Chip (DoC) with an HPS instance for motion control.
The name of this design is specified as "3x2" given there is three instance of NiosV/g processor controlling two drives axes each, hence 6 motor axes in total.
<br>
<br>  
...
**(Optional)** Build Toolchain for ARM processor. The recommended is toolchain is [gcc-arm-11.2-2022.02-x86_64-aarch64-none-linux-gnu.tar.xz](https://developer.arm.com/-/media/Files/downloads/gnu/11.2-2022.02/binrel/gcc-arm-11.2-2022.02-x86_64-aarch64-none-linux-gnu.tar.xz)
<br>
<br>
...
## Getting Started.
### a. Create the "6-Axis Drive-On-Chip based on NIOSV/g with Hard Processor System" with Quartus and Platform Designer Project based on the Modular Design Toolkit.
...
quartus_sh -t ./modular_design_toolkit/scripts/create/create_shell.tcl -xml_path ./HPS_NIOSVg_DoC_3x2_axis/AGX_5E_Modular_Devkit_HPS_NIOSVg_DoC_3x2_axis.xml -proj_path <project> -o
```
...
### b. Build the "6-Axis Drive-on-Chip based on NIOSV/g with Hard Processor System" Quartus and Platform Designer Project based on the Modular Design Toolkit.
...
The following block diagrams show the main components and subsystems of the "6-Axis Drive-On-Chip based on NIOSV/g with Hard Processor System".
The first diagram shows an instance of the "Dual-Axis Drive-On-Chip" with an AXI Interface intended for the HPS Host 
<br>
<br>

![image](https://github.com/user-attachments/assets/4dc78209-a45d-4c5c-b516-750d710ec669)

<br>
<br>
The next diagram shows three instances of the "Dual-Axis Drive-On-Chip" connected to the AXI Interface that allows the HPS to perform motion control in any of the axes.
<br>
<br>

![image](https://github.com/user-attachments/assets/f78fa0df-f0ae-4fb3-b0ce-60a32713df9c)

<br>
<br>
The next diagram is color-coded to match the Platform Designer view and the XML contained in the "designs" folder in this repository. The following figure correlates the block diagram, the XML file and the Platform Designer view:
<br>
<br>

![image](https://github.com/user-attachments/assets/dbd5a47b-e866-447d-a24b-50702621e8a9)
...
The resulting addressing is as the following:  Target = HPS_bridge_base + Axis_Base + Offset_from_Axis_Base_Reg.
<br>
