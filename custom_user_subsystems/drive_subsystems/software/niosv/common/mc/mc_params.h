...
/**
 * @brief Structure to encapsulate the GUI Trace parameters
 */
typedef struct {
    ...
    void *trigger_in_ptr;           // Pointer to selected data for trigger_in
    int  (*get_trigger_in)(void *); // Pointer to function to get trigger_in data as integer
...
struct system_params {
    ...
    // Struct storing parameters for GUI trace display
...
struct drive_params {
    ...
    int             speed_command;          //!<Speed request/command, scaled by SPEED_FRAC_BITS
...
/**
 * @brief Waveform Structure: Contain the settings for the waveform generator.
 */
typedef struct {
    demo_wave_e shape;
    int period;                          //!<Period in ms
    int offset;                          //!<Speed or position offset
    int mode;                            //!<Waveform mode: sine, trangle, square, saw, constant
    int cmd_wave_amp;                    //!<Waveform amplitude
} waveform_input_params_t;
