...
SRC_URI = "\
        ...
        file://fix-signal-handler-signature.patch \
...
do_install() {
    ...
    install -m 0755 ${UNPACKDIR}/q ${D}${bindir}

    case "${MACHINE}" in
        *"agilex7_"*)
            install -m 0755 ${UNPACKDIR}/start_agx7.sh ${D}${bindir}/start.sh
            ;;
        *"agilex5_"*)
            install -m 0755 ${UNPACKDIR}/start_agx5.sh ${D}${bindir}/start.sh
            ;;
        *)
            ;;
    esac
